...
export default class Gameboard {
  ...
  userPlaceShips() {
    ...
    const placeGrid = Array.from(
      document.querySelectorAll('div#place>div.grid')
    );
    // Displays which ship is to be placed as the cursor
    placeGrid.forEach((grid) => {
      grid.addEventListener('mouseover', (event) => {
        if (direction === 0)
          event.target.classList.add(`${shipQueue[0].type}Rotated`);
    ...
    placeGrid.forEach((grid) => {
      grid.addEventListener('mouseout', (event) => {
        if (shipQueue.length > 0) {
          if (direction === 0)
            event.target.classList.remove(`${shipQueue[0].type}Rotated`);
          if (direction === 1)
        ...
        if (
          ...
          for (let i = 0; i < ship.length; i++) {
            ...
            square =
              direction === 0
                ? this.findSquare(
                    `[${square.coordinates[0]},${square.coordinates[1] + 1}]`
                  )
                : this.findSquare(
                    `[${square.coordinates[0] + 1},${square.coordinates[1]}]`
                  );
          }
  ...
  findLegalSquares(ship) {
    ...
    do {
      ...
      squaresOccupied = this.checkSquaresAreOccupied(
        xCoordinate,
        yCoordinate,
        direction,
        ship.length
      ...
      squaresOccupied ||
      (direction === 0 && yCoordinate + ship.length > 9) ||
      (direction === 1 && xCoordinate + ship.length > 9)
    );
  ...
  findSquare(searchCoordinates) {
    return this.squares.find(
      (element) => JSON.stringify(element.coordinates) === searchCoordinates
    );
  }
  ...
  checkSquaresAreOccupied(xCoordinate, yCoordinate, direction, length) {
    for (let i = 0; i < length; i++) {
      const square =
        direction === 0
          ? this.findSquare(`[${xCoordinate},${yCoordinate + i}]`)
          : this.findSquare(`[${xCoordinate + i},${yCoordinate}]`);
